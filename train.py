...
from model.resfcn256 import Discriminator
from model.resfcn256 import Discriminator1
...
def main(data_dir):
    ...
    for ep in range(start_epoch, target_epoch):
        ...
        for i, sample in enumerate(bar):
            ...
            fake_loss = bce_loss(discriminator(uv_map_result).detach(), fake_label)
            ...
            logit.backward()
            optimizer.step()
            bar.set_description(" {} [Loss(Paper)] {} [Loss(D)] {} [SSIM({})] {}".format(ep, Loss_list[-1], Loss_list_D[-1],FLAGS["gauss_kernel"], Stat_list[-1]))
