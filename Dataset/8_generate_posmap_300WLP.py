...
def run_posmap_300W_LP(bfm, image_path, mat_path, save_folder,  uv_h = 256, uv_w = 256, image_h = 256, image_w = 256):
    ...
    # 5. save files
    io.imsave('{}/{}'.format(save_folder, "original.jpg"), np.squeeze(cropped_image))
    np.save('{}/{}'.format(save_folder, "uv_map.npy"), uv_position_map)
...
def run_posmap_helper(image_path, mat_path, save_folder):
    run_posmap_300W_LP(bfm, image_path, mat_path, save_folder,  uv_h = 450, uv_w = 450, image_h = 450, image_w = 450)
...
if __name__ == '__main__':
    ...
    # run
    folders = ["AFW", "AFW_Flip", "HELEN", "HELEN_Flip", "IBUG",
        "IBUG_Flip", "LFPW", "LFPW_Flip"]
    folders = ["AFW", "IBUG"]
    for i in folders:
        ...
        for k in tqdm(files):
            ...
            if not os.path.exists(save_path):
                os.mkdir(save_path)
            run_posmap_300W_LP(bfm, image_path, mat_path, save_path, 450, 450, 450, 450)
